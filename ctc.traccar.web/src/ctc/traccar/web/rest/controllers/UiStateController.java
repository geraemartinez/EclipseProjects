...
public class UiStateController {
	...
	public ResponseEntity<?> getUiStateById(@PathVariable(value="IdNumber") String id){
		...
		try {
			...
			UiStateVo vo =  service.getUiStateById(new UiStateVo(new BigInteger(id)));
			return new ResponseEntity<>(JsonParserHelper.toJson(vo),responseHeaders,HttpStatus.OK);
			
		}catch(Exception e){
			String error = "[ERROR] Uncaught Exception - Internal server error at getUiStateById: " + e.getMessage();
			String JsonError = "{ \"error\": \"" + error +"\"";
			logger.error(error);
			return new ResponseEntity<>(JsonError,HttpStatus.INTERNAL_SERVER_ERROR);			
		}
	...
	public ResponseEntity<?> getUserByUiState(@PathVariable(value="IdNumber") String id){
		...
		try {
			...
			UserVo vo =  service.getUserByUiState(new UiStateVo(new BigInteger(id)));
			return new ResponseEntity<>(JsonParserHelper.toJson(vo),responseHeaders,HttpStatus.OK);
			
		}catch(Exception e){
			String error = "[ERROR] Uncaught Exception - Internal server error at getUserByUiState: " + e.getMessage();
			String JsonError = "{ \"error\": \"" + error +"\"";
			logger.error(error);
			return new ResponseEntity<>(JsonError,HttpStatus.INTERNAL_SERVER_ERROR);			
		}
