...
public class GeofenceController {
	...
	public ResponseEntity<?> getAllGeofences(){
		...
		try {
			...
			List<GeofenceVo> vos =  service.getAllGeofences();
	...
	public ResponseEntity<?> getGeofenceById(@PathVariable(value="IdNumber") String id){
		
		HttpHeaders responseHeaders = new HttpHeaders();
		responseHeaders.add("Content-Type", "application/json; charset=utf-8");
		
		try {
			GeofenceVo vo  = new GeofenceVo(new BigInteger(id));
			vo =  service.getGeofenceById(vo);
			return new ResponseEntity<>(JsonParserHelper.toJson(vo),responseHeaders,HttpStatus.OK);
			
		}catch(Exception e){
			String error = "[ERROR] Uncaught Exception - Internal server error at getGeofenceById: " + e.getMessage();
			String JsonError = "{ \"error\": \"" + error +"\"";
			logger.error(error);
			return new ResponseEntity<>(JsonError,HttpStatus.INTERNAL_SERVER_ERROR);			
		}
	...
	public ResponseEntity<?> getDevicesGeofence(@PathVariable(value="IdNumber") String id){
		...
		try {
			...
			String error = "[ERROR] Uncaught Exception - Internal server error at getDevicesGeofence: " + e.getMessage();
			String JsonError = "{ \"error\": \"" + error +"\"";
			logger.error(error);
			return new ResponseEntity<>(JsonError,HttpStatus.INTERNAL_SERVER_ERROR);			
		}
