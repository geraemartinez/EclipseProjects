...
public class PictureController {
	...
	public ResponseEntity<?> getAllPicture(){
		...
		try {
			...
			List<PictureVo> vos =  service.getAllPicture();
			return new ResponseEntity<>(JsonParserHelper.toJson(vos),responseHeaders,HttpStatus.OK);
			
		}catch(Exception e){
			String error = "[ERROR] Uncaught Exception - Internal server error at getAllPicture: " + e.getMessage();
	...
	public ResponseEntity<?> getPictureById(@PathVariable(value="IdNumber") String id){
		...
		try {
			...
			PictureVo vo =  service.getPictureById(new PictureVo(new BigInteger(id)));
			return new ResponseEntity<>(JsonParserHelper.toJson(vo),responseHeaders,HttpStatus.OK);
			
		}catch(Exception e){
			String error = "[ERROR] Uncaught Exception - Internal server error at getPictureById: " + e.getMessage();
			String JsonError = "{ \"error\": \"" + error +"\"";
			logger.error(error);
			return new ResponseEntity<>(JsonError,HttpStatus.INTERNAL_SERVER_ERROR);			
		}
