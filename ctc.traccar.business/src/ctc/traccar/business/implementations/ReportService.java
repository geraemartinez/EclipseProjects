	...
	public List<GeofenceVo> getGeofenceByReport(ReportVo report) {
	  try {
			...
			if(report!=null && report.getId()!=null){
				...
				Report ToSearch = ReportHelper.converToObj(report);
				result =  GeofenceHelper.converToVos(repo.getGeofenceByReport(ToSearch));
				return result;
				
			}
			
		}catch(Exception e){ 
			logger.error("[ERROR] Uncaught Exception - the following error ocurred at ReportService getGeofenceByReport() method : " + e.getMessage());
		}
		return null;
	}
}
