...
public class PositionService implements PositionServiceInt {
	...
	public List<PositionVo> getAllPosition() {
		try {
			...
			List<Position> positions =  repo.getAllPosition();
			
			if (positions!=null && !positions.isEmpty()){
				
				result =  PositionHelper.converToVo(positions);
				return result;
	...
	public DeviceVo getDeviceByPosition(PositionVo position) {
		try {
			...
			if(position!=null && position.getId()!=null){
				...
				Position ToSearch = PositionHelper.converToObj(position);
				result = DeviceHelper.converToVO(repo.getDeviceByPosition(ToSearch));
				return result;
				
			}
			
		}catch(Exception e){ 
			logger.error("[ERROR] Uncaught Exception - the following error ocurred at PositionService getDeviceByPosition() method : " + e.getMessage());
		}
		return null;
	}
